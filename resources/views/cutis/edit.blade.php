        ...
        <div class="row my-3">
            <div class="col-md-12">
              <div class="card">
                ...
                <div class="card-body">
                  <form action="{{ route('cutis.update', $item->id_cuti) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        ...
                        <select name="user_id" id="user_id" class="form-control select2" required>
                            ...
                            <option value="{{ $users->id }}" {{ $users->id == $item->user_id ? 'selected' : '' }}>
                                {{ $users->name }}
                            </option>
                        @endforeach
                        </select>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="input-group input-group-outline w-100">
                                        <input type="text" value="{{ old('durasi_cuti') ?? $item->durasi_cuti }}" name="durasi_cuti" class="form-control" id="durasi_cuti" placeholder="Masukkan durasi cuti" readonly>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="input-group input-group-outline w-100">
                                        <input type="month" value="{{ old('tanggal_pengajuan') ?? $item->tanggal_pengajuan }}" name="tanggal_pengajuan" class="form-control" id="tanggal_pengajuan" placeholder="Masukkan tanggal pengajuan"  min="<?php echo $nextMonthDate; ?>" max="<?php echo $endMonthDate; ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab" tabindex="0">
                            <div class="row">
                                <div class="col-md-6 row">
                                    ...
                                    <input type="input" name="tanggal_pengajuan2" value="{{ old('tanggal_pengajuan2') ?? $item->tanggal_pengajuan2 }}" class="form-control d-none" id="tanggal_pengajuan123" placeholder="Masukkan tanggal_pengajuan">
                                </div>
                                <div class="col-md-6">
                                    <div class="input-group input-group-outline w-100">
                                        <input type="text" value="{{ old('durasi_cuti2') ?? $item->durasi_cuti2 }}" name="durasi_cuti2" class="form-control" id="total_cuti" placeholder="Masukkan durasi cuti">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="input-group input-group-outline w-100">
                        <select class="form-control w-100" name="status" id="status">
                            <option value="" disabled {{ !$item->status ? 'selected' : '' }}>Masukkan status</option>
                            <option value="active" {{ $item->status == 'active' ? 'selected' : '' }}>Active</option>
                            <option value="pending" {{ $item->status == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="declined" {{ $item->status == 'declined' ? 'selected' : '' }}>Declined</option>
                            <option value="non-active" {{ $item->status == 'non-active' ? 'selected' : '' }}>Non-active</option>
                        </select>
                    </div>
