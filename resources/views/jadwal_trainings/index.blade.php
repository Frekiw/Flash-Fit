...
<div class="page-wrapper mdc-toolbar-fixed-adjust">
  ...
  <main class="content-wrapper">
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('jadwal_trainings.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group mt-3">
                        ...
                        <select name="trainer_id" id="trainer_id" class="form-control select2">
                            ...
                            @foreach($participant as $trainer)
                            <option value="{{ $trainer->id_participant }}">
                                {{ $trainer->name }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="mdc-card">
                    ...
                    <div class="table-responsive">
                        <table id="example" class="table display">
                            ...
                            <tbody>
                              @forelse ($jadwal_training as $jadwal_trainings)
                                <tr>
                                    <td class="text-center">{{ $jadwal_trainings->id_jadwaltraining }}</td>
                                    <td class="text-center">
                                      {{ $jadwal_trainings->user->name }}
                                    </td>
                                    <td class="text-center">
                                      {{ $jadwal_trainings->detail_sesi }}
                                    </td>
                                    <td class="text-center">
                                      {{ $jadwal_trainings->schedule }}
                                    </td>
                                    <td class="text-center">
                                      @if ($jadwal_trainings->trainer_id == null)
                                      <div class="text-secondary">
                                        Belum Ada
                                      </div>
                                      @else
                                      <div class="btn btn-primary rounded-pill">
                                        <small>{{ $jadwal_trainings->participant->name }}</small>
                                      </div>
                                      @endif
                                    </td>
                                    <td class="text-center">
                                        {{ $jadwal_trainings->sisa_sesi }}
                                      </td>
                                    <td>
                                     <div class="d-flex justify-content-center align-items-center">
                                      <a type="button" data-bs-toggle="modal" data-bs-target="#EditModal{{ $jadwal_trainings->id_jadwaltraining }}" class="mdc-button mdc-button--outlined shaped-button outlined-button--edit mdc-ripple-upgraded mx-2"> Edit </a>
                                      <form action="{{ route('jadwal_trainings.destroy',$jadwal_trainings->id_jadwaltraining) }}" onsubmit="return confirm('Apakah Anda Ingin Menghapus Data Ini?')" method="POST">
                                        {!! method_field('DELETE') !!}
                                        {!! csrf_field() !!}
    ...
    @foreach ($jadwal_training as $jadwal_trainings)
    <div class="modal fade" id="EditModal{{ $jadwal_trainings->id_jadwaltraining }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
              <div class="modal-header">
              ...
              <div class="modal-body">
                <form action="{{ route('jadwal_trainings.update', $jadwal_trainings->id_jadwaltraining) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group mt-3">
                    <label for="user_id">Nama Member</label>
                    <select name="user_id" id="user_id" class="form-control select2" required>
                        <option value="">Pilih Member</option>
                        @foreach($user as $usr)
                        <option value="{{ $usr->id }}" {{ $jadwal_trainings->user_id == $usr->id ? 'selected' : '' }}>
                            {{ $usr->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="input-group input-group-outline w-100">
                    <select class="form-control w-100" name="detail_sesi" id="detail_sesi">
                        <option value="" disabled selected>Masukkan Detail Sesi</option>
                        <option value="Basic 1 (2 Sesi)" {{ $jadwal_trainings->detail_sesi == 'Basic 1 (2 Sesi)' ? 'selected' : '' }}>Basic 1 (2 Sesi)</option>
                        <option value="Basic 2 (2 Sesi)" {{ $jadwal_trainings->detail_sesi == 'Basic 2 (2 Sesi)' ? 'selected' : '' }}>Basic 2 (2 Sesi)</option>
                    </select>
                </div>
                <label for="notes" class=" mt-3">Jadwal Training</label>
                <textarea class="form-control" type="text" value="{{ old('schedule') ?? $jadwal_trainings->schedule }}" id="schedule" name="schedule">{{ $jadwal_trainings->schedule }}</textarea>
                <div class="form-group mt-3">
                    <label for="trainer_id">Nama Trainer</label>
                    <select name="trainer_id" id="trainer_id" class="form-control select2">
                        <option value="">Pilih trainer</option>
                        @foreach($participant as $trainer)
                        <option value="{{ $trainer->id_participant }}" {{ $jadwal_trainings->trainer_id == $trainer->id_participant ? 'selected' : '' }}>
                            {{ $trainer->name }}
                        </option>
                        @endforeach
                    </select>
